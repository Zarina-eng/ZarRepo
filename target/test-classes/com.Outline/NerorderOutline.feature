Feature: Validation of New order using Scenario Outline
  ...
  Scenario Outline: New Order Validation
    ...
    * the user enter payment info "<card>", "<cardNumber>", "<expDate>"
    Then the user validate  success message
    And  the user validate new order details "<productName>", "<quantity>", "<name>", "<address>", "<city>", "<state>", "<zipCode>", "<card>", "<cardNumber>", "<expDate>"
    Examples:
      | productName | quantity | name    | address      | city        | state      | zipCode | card             | cardNumber       | expDate |
